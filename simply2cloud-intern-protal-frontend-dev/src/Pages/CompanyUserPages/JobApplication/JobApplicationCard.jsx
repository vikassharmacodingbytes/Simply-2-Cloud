...
function JobApplicationCard({ jobApplication }) {
...
console.log(jobApplication)
  ...
  <InternProfileCard profile={jobApplication.intern_job_profile}/>
  <div>
    <div className='text-gray-700 font-semibold'>
        <h1><span className='font-bold'>Applied For {" "}</span><span> {jobApplication?.job?.job_categoery?.job_category} <span className='text-xs'>({jobApplication?.job?.sub_categoery?.sub_category_name})</span></span></h1>
    </div>
  </div>
...
<Button variant='outlined' color='info' onClick={()=>{
       ...
       {approveBtn ? <CircularProgress size={19} color='inherit'/> :<> Approve 
        {/* <Check /> */}
        </>}
        </Button> &nbsp;&nbsp;&nbsp;&nbsp;
        <Button variant='outlined' color='error'onClick={()=>{
          ...
          {deleteBtn ? <CircularProgress size={19} color='error'/> :<> Reject
            {/* <Close /> */}
            </>}
